...
class SuperShallowDeepUConvKAN(nn.Module):
    """

    """
    def __init__(self, channels, hidden_channels, n_classes=1, BN=True):
        super(SuperShallowDeepUConvKAN, self).__init__()

        self.doble_conv = DobleConv(channels, 2 * hidden_channels, hidden_channels, BN)
        self.down_block1 = DownBlock(hidden_channels, 2 * hidden_channels, hidden_channels, BN, kernel_size=8)
        ...
        self.up_block1 = UpBlock(hidden_channels, hidden_channels, 2 * hidden_channels, hidden_channels, BN, kernel_size=8)
