...
class Experiment(pl.LightningModule):
    ...
    def training_step(self, input, idx):
        ...
        self.fit_metrics['train'].update(pred=output['pred'], ref=gt['gt'])
    ...
    def validation_step(self, input, idx):
        ...
        output = self.forward(low)
        loss, loss_dict = self.loss_criterion(output, gt)
        self.fit_metrics['validation'].update(pred=output['pred'], ref=gt['gt'])
        self.loss_report(loss, loss_dict,'validation')
        return loss
    def test_step(self, batch, batch_idx, dataloader_idx=0):
        low, gt, name = batch
        output = self.forward(low)
        self.eval_metrics[self.eval_subsets[dataloader_idx]].update(pred=output['pred'], ref=gt['gt'])
